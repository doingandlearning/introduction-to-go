# Lab 12: Testing — writing tests for the shipping package
...
is the tests. `starter/internal/shipping/shipping_test.go` has real test
...
Both directories share the same shape: an `internal/shipping` package
(the code under test) and a `cmd/shippingcli` package (a runnable demo,
...
**Context:** `starter/internal/shipping/shipping_test.go` has
`TestExpressSurcharge` stubbed out with `t.Skip`.

**Tasks:**

1. Replace the `t.Skip` line with a real test: call
   `shipping.ExpressSurcharge(80)`, compare it to `92.0`, and call
   `t.Errorf` if they don't match — following the exact shape from the
   lecture.
2. Run `go test ./...` from `starter/`. Confirm it passes.
3. Open `starter/internal/shipping/shipping.go` and change
   `ExpressSurcharge` to multiply by `1.10` instead of `1.15`. Run
   `go test ./...` again and read the failure output carefully.
4. Put `1.15` back and confirm the test passes again.
...
`shipping.go` that caused it. Diagnosing the cause is still on you; the
...
**Context:** `CostPerParcel` in `shipping.go` panics instead of
returning an error when the parcel count is zero.
`TestCostPerParcelZeroPanics` is stubbed out in the starter.

**Tasks:**

1. Replace the `t.Skip` in `TestCostPerParcelZeroPanics` with a real
   implementation: `defer` an anonymous function that calls `recover()`
   and calls `t.Error(...)` if the result is `nil`, then call
   `CostPerParcel(90, 0)` as the last line of the test.
2. Run `go test ./...`. Confirm it passes.
3. Temporarily remove the `panic(...)` call from `CostPerParcel` in
   `shipping.go` (just make it return `0` for the zero case instead).
   Run `go test ./...` again — confirm `TestCostPerParcelZeroPanics` now
   *fails*.
...
propagates up out of `CostPerParcel`, the only code still running is
...
**Context:** `ZoneRate` in `shipping.go` has four branches based on
parcel weight. `TestZoneRate` is stubbed out in the starter.

**Tasks:**

1. Replace the `t.Skip` in `TestZoneRate` with a table: a slice of an
   anonymous struct with `name`, `weight`, and `want` fields. Include
...
4. Pick one case name from your table and run only that case with:
   `go test -run 'TestZoneRate/<case name, spaces as underscores>'`
...
it's likely your table doesn't exercise every branch of `ZoneRate`.
...
2. Run `go test -coverprofile=cover.out ./...` followed by
   ...
   `shipping.go` with covered lines in green and uncovered lines in
   red.
3. Find at least one red (uncovered) line inside `ZoneRate`. Write a
   new test function (or add a case to your table) that exercises it.
4. Re-run the coverage report and confirm that line is now green.
...
**Context:** `BenchmarkExpressSurcharge` is stubbed out in the starter
with `b.Skip`.

**Tasks:**

1. Replace the `b.Skip` line with a loop:
   `for i := 0; i < b.N; i++ { shipping.ExpressSurcharge(80) }` (adjust
...
2. Run `go test -bench=. -benchmem ./...`. Note the reported `ns/op`
   ...
   `allocs/op`).
3. Make a small, deliberate change to `ExpressSurcharge` — for example,
   temporarily return `cost + (cost * 0.15)` instead of `cost * 1.15`
   — and re-run the benchmark. Do the numbers move? Put the original
...
1. Look at `solution/internal/shipping/testify_example.go.txt` — a
   version of `TestExpressSurcharge` rewritten with `assert.Equal`
   instead of a manual `if`/`t.Errorf`. (It's named `.go.txt`, not
   `_test.go`, so the lab compiles offline without needing to fetch the
   dependency — see the comment at the top of that file for how to
   actually run it if you have network access.)
